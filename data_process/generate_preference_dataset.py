...
def build_prompt(company_info, research_trends, achievements):
    return [
        {"role": "system", "content":
            ...
            f"2. 对于可采纳预测，必须按【方向一】、【方向二】、【方向三】逐条返回，并直接从“【方向一】”开始输出，禁止添加其余任何非条目内容的标题、前言或总结；并且每个方向都必须严格按照以下输出结构的格式生成："
            ...
            f"3. 对于不可采纳预测，每个方向必须用“1.”, “2.”, “3.”这样的编号进行标记，并且每个方向都必须严格按照以下输出结构的格式生成："
            f"序号. 方向名称（用一句话凝练地概括R&D方向）（简要说明为何该方向不可采纳，指出其缺乏前瞻性、技术可行性或与企业业务的关联性）。"
            ...
            f"【不可采纳预测】\n"
            f"1. 基于区块链的火星殖民物联网供应链溯源（过于宽泛）2. 基于Windows XP内核的工业互联网实时操作系统研发（过时技术方向）3. 面向传统手摇纺车的5G边缘计算智能纱锭改造（与企业行业无关）"
